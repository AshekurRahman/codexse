...
<div x-data="liveChatDashboard()" class="p-6">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-surface-900 dark:text-white">Live Chat Support</h1>
        <p class="text-surface-600 dark:text-surface-400 mt-1">Manage incoming customer chat requests</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white dark:bg-surface-800 rounded-xl shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-surface-200 dark:border-surface-700 flex items-center justify-between">
                <h2 class="font-semibold text-surface-900 dark:text-white flex items-center">
            ...
            <div class="divide-y divide-gray-200 dark:divide-gray-700 max-h-[500px] overflow-y-auto">
                <template x-for="chat in waitingChats" :key="chat.id">
                    <div class="p-4 hover:bg-surface-50 dark:hover:bg-surface-700/50 cursor-pointer transition-colors" @click="acceptChat(chat)">
                        <div class="flex items-start justify-between mb-2">
                            <div class="flex items-center space-x-2">
                                <div class="w-8 h-8 rounded-full bg-surface-200 dark:bg-surface-600 flex items-center justify-center">
                                    <span class="text-sm font-medium text-surface-600 dark:text-surface-300" x-text="(chat.visitor_name || 'V').charAt(0).toUpperCase()"></span>
                                </div>
                                <div>
                                    <p class="font-medium text-surface-900 dark:text-white text-sm" x-text="chat.visitor_name || 'Visitor'"></p>
                                    <p class="text-xs text-surface-500 dark:text-surface-400" x-text="chat.department"></p>
                                </div>
                            </div>
                            <span class="text-xs text-surface-500" x-text="formatTime(chat.created_at)"></span>
                        </div>
                        <p class="text-sm text-surface-600 dark:text-surface-400 truncate" x-text="chat.subject || 'No subject'"></p>
                ...
                <div x-show="waitingChats.length === 0" class="p-8 text-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 mx-auto text-surface-300 dark:text-surface-600 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                    </svg>
                    <p class="text-surface-500 dark:text-surface-400">No waiting chats</p>
                </div>
            </div>
        </div>

        <!-- Active Chat Window -->
        <div class="lg:col-span-2 bg-white dark:bg-surface-800 rounded-xl shadow-sm overflow-hidden flex flex-col" style="min-height: 600px;">
            <!-- No Active Chat -->
            <div x-show="!activeChat" class="flex-1 flex items-center justify-center">
                <div class="text-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 mx-auto text-surface-300 dark:text-surface-600 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a1.994 1.994 0 01-1.414-.586m0 0L11 14h4a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2v4l.586-.586z" />
                    </svg>
                    <h3 class="text-lg font-medium text-surface-900 dark:text-white mb-1">No Active Chat</h3>
                    <p class="text-surface-500 dark:text-surface-400">Select a waiting chat to start helping a customer</p>
                </div>
            </div>
            ...
            <template x-if="activeChat">
                <div class="flex flex-col h-full">
                    <!-- Chat Header -->
                    <div class="px-6 py-4 border-b border-surface-200 dark:border-surface-700 flex items-center justify-between bg-surface-50 dark:bg-surface-700/50">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <p class="font-medium text-surface-900 dark:text-white" x-text="activeChat.visitor_name || 'Visitor'"></p>
                                <p class="text-sm text-surface-500 dark:text-surface-400" x-text="activeChat.visitor_email"></p>
                            </div>
                        </div>
                        <div class="flex items-center space-x-2">
                            <select x-model="transferDepartment" class="text-sm border border-surface-300 dark:border-surface-600 rounded-lg px-3 py-1.5 bg-white dark:bg-surface-700 text-surface-900 dark:text-white">
                            ...
                            <button @click="transferChat" :disabled="!transferDepartment" class="px-3 py-1.5 text-sm border border-surface-300 dark:border-surface-600 rounded-lg hover:bg-surface-50 dark:hover:bg-surface-600 disabled:opacity-50 text-surface-700 dark:text-surface-300">
                                Transfer
                            </button>
                        ...
                        <template x-for="message in messages" :key="message.id">
                            <div :class="message.sender_type === 'agent' ? 'flex justify-end' : 'flex justify-start'">
                                ...
                                <div x-show="message.sender_type === 'visitor'" class="max-w-[70%]">
                                    <div class="bg-surface-100 dark:bg-surface-700 rounded-2xl rounded-tl-none px-4 py-2">
                                        <p class="text-surface-900 dark:text-white text-sm whitespace-pre-wrap" x-text="message.message"></p>
                                    </div>
                                    <span class="text-xs text-surface-500 mt-1 ml-2" x-text="formatTime(message.created_at)"></span>
                                </div>
                                ...
                                <div x-show="message.sender_type === 'agent'" class="max-w-[70%]">
                                    ...
                                    <span class="text-xs text-surface-500 mt-1 mr-2 text-right block" x-text="formatTime(message.created_at)"></span>
                                </div>
                            </div>
                        </template>
                    </div>
                    ...
                    <!-- Quick Responses -->
                    <div class="px-4 py-2 border-t border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-700/50">
                        <div class="flex flex-wrap gap-2">
                            <template x-for="response in quickResponses" :key="response.id">
                                <button
                                    @click="useQuickResponse(response)"
                                    class="text-xs px-3 py-1 bg-white dark:bg-surface-700 border border-surface-300 dark:border-surface-600 rounded-full hover:bg-surface-100 dark:hover:bg-surface-600 text-surface-700 dark:text-surface-300 transition-colors"
                    ...
                    <div class="p-4 border-t border-surface-200 dark:border-surface-700">
                        <form @submit.prevent="sendMessage" class="flex items-center space-x-3">
                            <input
                                ...
                                :disabled="sending"
                                class="flex-1 px-4 py-2.5 border border-surface-300 dark:border-surface-600 rounded-lg bg-white dark:bg-surface-700 text-surface-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
    ...
    <!-- My Active Chats -->
    <div class="mt-6 bg-white dark:bg-surface-800 rounded-xl shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-surface-200 dark:border-surface-700">
            <h2 class="font-semibold text-surface-900 dark:text-white">My Active Chats</h2>
        </div>
        <div class="divide-y divide-gray-200 dark:divide-gray-700">
            <template x-for="chat in myActiveChats" :key="chat.id">
                <div
                    ...
                    class="p-4 hover:bg-surface-50 dark:hover:bg-surface-700/50 cursor-pointer transition-colors flex items-center justify-between"
                >
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <p class="font-medium text-surface-900 dark:text-white" x-text="chat.visitor_name || 'Visitor'"></p>
                            <p class="text-sm text-surface-500 dark:text-surface-400" x-text="chat.department"></p>
                        </div>
                    </div>
                    <div class="text-right">
                        ...
                        <p class="text-xs text-surface-500 mt-1" x-text="formatTime(chat.updated_at)"></p>
                    </div>
                </div>
            </template>
            <div x-show="myActiveChats.length === 0" class="p-8 text-center">
                <p class="text-surface-500 dark:text-surface-400">No active chats</p>
            </div>
        </div>
    </div>
</div>
