            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <aside class="lg:col-span-1">
                    <nav class="rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-800 overflow-hidden sticky top-24">
                        ...
                        <a href="{{ route('subscriptions.manage') }}" class="flex items-center gap-3 px-4 py-3 text-surface-600 dark:text-surface-400 hover:bg-surface-50 dark:hover:bg-surface-700 border-l-4 border-transparent transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="lg:col-span-3 space-y-6">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        ...
                        <div class="rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-800 p-5">
                            <div class="flex items-center justify-between mb-3">
                                ...
                                <span class="text-xs font-medium text-warning-600 dark:text-warning-400 bg-warning-100 dark:bg-warning-900/30 px-2 py-1 rounded-full">Recurring</span>
                            </div>
                            <p class="text-3xl font-bold text-surface-900 dark:text-white">{{ $activeSubscriptions ?? 0 }}</p>
                            <p class="text-sm text-surface-500 dark:text-surface-400 mt-1">Subscriptions</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div class="rounded-xl border border-surface-200 dark:border-surface-700 bg-gradient-to-br from-primary-500 to-primary-700 p-5 text-white">
                            ...
                            <p class="text-3xl font-bold">{{ format_price($totalSpent ?? 0) }}</p>
                        ...
                        <div class="rounded-xl border border-surface-200 dark:border-surface-700 bg-gradient-to-br from-accent-500 to-accent-700 p-5 text-white">
                            ...
                            <p class="text-3xl font-bold">{{ format_price($totalServiceSpent ?? 0) }}</p>
                            <p class="text-sm text-accent-200 mt-1">Lifetime total</p>
                        </div>
                        <a href="{{ route('wallet.index') }}" class="rounded-xl border border-surface-200 dark:border-surface-700 bg-gradient-to-br from-success-500 to-success-700 p-5 text-white hover:from-success-600 hover:to-success-800 transition-all">
                            <div class="flex items-center justify-between mb-2">
                                <p class="text-sm font-medium text-success-100">Wallet Balance</p>
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-success-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                                </svg>
                            </div>
                            <p class="text-3xl font-bold">{{ format_price($wallet->balance ?? 0) }}</p>
                            <p class="text-sm text-success-200 mt-1">Available funds</p>
                        </a>
                    </div>
                                        ...
                                        <div class="px-5 py-3 hover:bg-surface-50 dark:hover:bg-surface-700/50 transition-colors">
                                            <div class="flex items-center justify-between">
                                                ...
                                                <div class="text-right">
                                                    <p class="font-semibold text-surface-900 dark:text-white text-sm">{{ format_price($order->total) }}</p>
                    ...
                    @if(isset($recentWalletTransactions) && $recentWalletTransactions->count() > 0)
                        <div class="rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-800 overflow-hidden">
                            <div class="flex items-center justify-between px-5 py-4 border-b border-surface-200 dark:border-surface-700">
                                <h2 class="font-semibold text-surface-900 dark:text-white">Recent Wallet Activity</h2>
                                <a href="{{ route('wallet.transactions') }}" class="text-sm text-primary-600 dark:text-primary-400 hover:text-primary-700 font-medium">View All</a>
                            </div>
                            <div class="divide-y divide-surface-200 dark:divide-surface-700">
                                        ...
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center gap-3">
                                                <div class="flex h-10 w-10 items-center justify-center rounded-full
                                                    @if($transaction->amount > 0) bg-success-100 dark:bg-success-900/30 text-success-600 dark:text-success-400
                                                    @else bg-danger-100 dark:bg-danger-900/30 text-danger-600 dark:text-danger-400
                                                    @endif">
                                                ...
                                                <div>
                                                    <p class="font-medium text-surface-900 dark:text-white text-sm">{{ ucfirst(str_replace('_', ' ', $transaction->type)) }}</p>
                                                    <p class="text-xs text-surface-500 dark:text-surface-400">{{ $transaction->created_at->format('M d, Y h:i A') }}</p>
                                                </div>
                                            </div>
                                            <div class="text-right">
                                                <p class="font-semibold text-sm @if($transaction->amount > 0) text-success-600 dark:text-success-400 @else text-danger-600 dark:text-danger-400 @endif">
                                                    {{ $transaction->amount > 0 ? '+' : '-' }}{{ format_price(abs($transaction->amount)) }}
                                                </p>
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium
                                                    @if($transaction->status === 'completed') bg-success-100 text-success-800 dark:bg-success-900/30 dark:text-success-400
                                                    @elseif($transaction->status === 'pending') bg-warning-100 text-warning-800 dark:bg-warning-900/30 dark:text-warning-400
                    ...
                    @if(isset($recentSubscriptions) && $recentSubscriptions->count() > 0)
                        <div class="rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-800 overflow-hidden">
                            <div class="flex items-center justify-between px-5 py-4 border-b border-surface-200 dark:border-surface-700">
                                <h2 class="font-semibold text-surface-900 dark:text-white">Active Subscriptions</h2>
                                <a href="{{ route('subscriptions.manage') }}" class="text-sm text-primary-600 dark:text-primary-400 hover:text-primary-700 font-medium">Manage All</a>
                            </div>
                            <div class="divide-y divide-surface-200 dark:divide-surface-700">
                                @foreach($recentSubscriptions->take(5) as $subscription)
                                    <a href="{{ route('subscriptions.subscription', $subscription) }}" class="block px-5 py-3 hover:bg-surface-50 dark:hover:bg-surface-700/50 transition-colors">
                                        <div class="flex items-center justify-between">
                                            <div class="min-w-0 flex-1">
                                                <p class="font-medium text-surface-900 dark:text-white text-sm truncate">{{ $subscription->plan->name }}</p>
                                            ...
                                            <div class="text-right ml-4">
                                                <p class="font-semibold text-surface-900 dark:text-white text-sm">{{ format_price($subscription->plan->price) }}/{{ $subscription->plan->billing_period }}</p>
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium
