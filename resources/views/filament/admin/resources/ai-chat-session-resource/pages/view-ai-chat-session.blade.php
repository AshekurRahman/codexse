        ...
        <x-filament::section>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    @if($record->user)
                        <p class="text-sm text-surface-500">{{ $record->user->email }}</p>
                    @elseif($record->guest_email)
                        <p class="text-sm text-surface-500">{{ $record->guest_email }}</p>
                    @else
                        <p class="text-sm text-surface-400 italic">No email provided</p>
                    @endif
                </div>
                <div>
                ...
                <div>
                    <span class="text-sm text-surface-500 dark:text-surface-400">Statistics:</span>
                    <p class="text-sm">{{ $record->message_count }} messages</p>
                    <p class="text-sm">{{ number_format($record->total_tokens_used) }} tokens used</p>
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 pt-4 border-t border-surface-200 dark:border-surface-700">
                <div>
                    <span class="text-sm text-surface-500 dark:text-surface-400">Session ID:</span>
                    <p class="text-sm font-mono text-surface-600 dark:text-surface-300">{{ $record->session_id }}</p>
                </div>
                <div>
                    <span class="text-sm text-surface-500 dark:text-surface-400">Started:</span>
            ...
            <div class="space-y-4 max-h-[600px] overflow-y-auto">
                        ...
                        <div class="shrink-0">
                            ...
                            @else
                                <div class="w-10 h-10 rounded-full bg-surface-100 dark:bg-surface-800 flex items-center justify-center">
                                    <x-heroicon-o-user class="w-5 h-5 text-surface-600 dark:text-surface-400" />
                                </div>
                            @endif
                        </div>
                        <div class="flex-1 max-w-[80%]">
                            <div class="p-4 rounded-lg {{ $message->role === 'assistant' ? 'bg-primary-50 dark:bg-primary-900/20' : 'bg-surface-50 dark:bg-surface-800' }}">
                                <div class="flex justify-between items-start mb-2">
                                    <span class="font-medium text-sm">
                                        {{ $message->role === 'assistant' ? 'AI Assistant' : $record->display_name }}
                                    </span>
                                    <span class="text-xs text-surface-500">{{ $message->created_at->format('M j, Y g:i A') }}</span>
                                </div>
                ...
                @empty
                    <div class="text-center py-8">
                        <x-heroicon-o-chat-bubble-bottom-center-text class="w-12 h-12 mx-auto text-surface-400 mb-3" />
                        <p class="text-surface-500 dark:text-surface-400">No messages in this conversation.</p>
                    </div>
                @endforelse
            </div>
        </x-filament::section>
