        ...
        <template x-if="results.products.length > 0">
            <div>
                ...
                <template x-for="(product, index) in results.products" :key="'prod-' + product.id">
                    ...
                    >
                        <div class="w-12 h-12 rounded-lg bg-surface-100 dark:bg-surface-700 overflow-hidden shrink-0 flex items-center justify-center">
                            <img x-show="product.thumbnail" :src="product.thumbnail" :alt="product.name" class="w-full h-full object-cover">
                            <svg x-show="!product.thumbnail" class="w-6 h-6 text-surface-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" x-cloak>
