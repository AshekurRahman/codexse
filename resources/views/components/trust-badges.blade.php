@props(['variant' => 'default', 'badges' => null])

@php
    use App\Models\TrustBadge;

    // Fetch trust badges from database
    $badges = $badges ?? TrustBadge::getForHomepage();

    // Icon map for Heroicons
    $iconSvgs = [
...
<section class="py-12 bg-white dark:bg-surface-900 border-y border-surface-100 dark:border-surface-800">
    <div class="mx-auto max-w-7xl container-padding">
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-{{ min($badges->count(), 6) }} gap-8 items-center">
            @foreach($badges as $badge)
            <div class="trust-badge group" x-scroll-animate>
                <div class="trust-badge-icon w-16 h-16 rounded-2xl bg-{{ $badge->icon_color }}-100 dark:bg-{{ $badge->icon_color }}-900/30 flex items-center justify-center mb-3">
                    <svg class="w-8 h-8 text-{{ $badge->icon_color }}-600 dark:text-{{ $badge->icon_color }}-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        {!! $iconSvgs[$badge->icon] ?? $iconSvgs['check-circle'] !!}
                    </svg>
                </div>
                <span class="text-sm font-semibold text-surface-900 dark:text-white">{{ $badge->title }}</span>
                @if($badge->subtitle)
                <span class="text-xs text-surface-500 dark:text-surface-400 mt-1">{{ $badge->subtitle }}</span>
                @endif
            </div>
            @endforeach
        </div>
