<div x-data="liveChatWidget()" x-cloak class="fixed bottom-4 right-4 z-50">
    ...
    >
        ...
        ></span>
        <!-- Tooltip -->
        <span class="absolute right-16 bg-surface-900 text-white text-sm px-3 py-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
            Chat with us
        </span>
    </button>
    ...
    >
        ...
        <div x-show="!chat && !loading" class="flex-1 overflow-y-auto p-4">
            <div class="text-center mb-6">
                ...
                <h4 class="text-lg font-semibold text-surface-900 dark:text-white">Start a Conversation</h4>
                <p class="text-sm text-surface-600 dark:text-surface-400 mt-1">We're here to help you with any questions</p>
            </div>
            ...
            <form @submit.prevent="startChat" class="space-y-4">
                ...
                <div>
                    ...
                    <input
                        ...
                        x-model="form.name"
                        required
                        class="w-full px-4 py-2.5 border border-surface-300 dark:border-surface-600 rounded-lg bg-white dark:bg-surface-700 text-surface-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                ...
                <div>
                    <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-1">How can we help?</label>
                    <input
                        type="text"
        ...
        <div x-show="loading" class="flex-1 flex items-center justify-center">
            <div class="text-center">
                ...
                <p class="text-surface-600 dark:text-surface-400">Loading chat...</p>
            </div>
        </div>
        ...
        >
            <template x-for="message in messages" :key="message.id">
                <div :class="message.sender_type === 'visitor' ? 'flex justify-end' : 'flex justify-start'">
                    ...
                    <div x-show="message.sender_type === 'system'" class="w-full text-center">
                        <span class="text-xs text-surface-500 dark:text-surface-400 bg-surface-100 dark:bg-surface-700 px-3 py-1 rounded-full" x-text="message.message"></span>
                    </div>
                    ...
                    <div x-show="message.sender_type === 'agent'" class="max-w-[80%]">
                        <div class="flex items-start space-x-2">
                            ...
                            <div>
                                ...
                                <span class="text-xs text-surface-500 dark:text-surface-400 mt-1 ml-2" x-text="formatTime(message.created_at)"></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-show="message.sender_type === 'visitor'" class="max-w-[80%]">
                        ...
                        <span class="text-xs text-surface-500 dark:text-surface-400 mt-1 mr-2 text-right block" x-text="formatTime(message.created_at)"></span>
                    </div>
                </div>
            </template>
            ...
            <div x-show="agentTyping" class="flex justify-start">
                <div class="bg-surface-100 dark:bg-surface-700 rounded-2xl px-4 py-3">
        ...
        <!-- Chat Input -->
        <div x-show="chat && !loading && chat.status !== 'closed'" class="border-t border-surface-200 dark:border-surface-700 p-4">
            <form @submit.prevent="sendMessage" class="flex items-center space-x-2">
                <input
                    ...
                    :disabled="sending"
                    class="flex-1 px-4 py-2.5 border border-surface-300 dark:border-surface-600 rounded-full bg-white dark:bg-surface-700 text-surface-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:opacity-50"
        ...
        <!-- Chat Closed State -->
        <div x-show="chat && chat.status === 'closed'" class="border-t border-surface-200 dark:border-surface-700 p-4">
            <div x-show="!showRating" class="text-center">
                <p class="text-surface-600 dark:text-surface-400 text-sm mb-3">This chat has ended</p>
            ...
            <!-- Rating Form -->
            <div x-show="showRating" class="text-center">
                <p class="text-surface-700 dark:text-surface-300 font-medium mb-3">How was your experience?</p>
                <div class="flex justify-center space-x-1 mb-4">
                    <template x-for="star in 5" :key="star">
                        <button
                            @click="rating = star"
                            :class="star <= rating ? 'text-yellow-400' : 'text-surface-300'"
