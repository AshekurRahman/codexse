...
class SocialLoginSetting extends Model
{
    ...
    {
        $setting = static::getProvider($provider);
        return $setting && $setting->is_enabled && $setting->client_id && $setting->client_secret;
    }
    ...
    public static function getEnabledProviders(): array
    {
        return static::enabled()
            ->ordered()
            ->get()
            ->filter(fn($s) => $s->client_id && $s->client_secret)
            ->pluck('provider')
            ->toArray();
    }
    ...
    public static function getProviderIcon(string $provider): string
    {
        return match($provider) {
            ...
            default => '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm0 22c-5.523 0-10-4.477-10-10S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/></svg>',
        };
    }
}
